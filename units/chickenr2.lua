-- UNITDEF -- CHICKENR2 --
--------------------------------------------------------------------------------

local unitName = "chickenr2"

--------------------------------------------------------------------------------

local unitDef = {
  acceleration       = 0.89999997615814,
  activateWhenBuilt  = false,
  bmcode             = 1,
  brakeRate          = 0.69999998807907,
  buildCostEnergy    = 15000,
  buildCostMetal     = 250,
  builder            = false,
  buildTime          = 3000,
  canGuard           = false,
  canMove            = false,
  canPatrol          = false,
  canstop            = 1,
  category           = [[MOBILE WEAPON NOTAIR NOTSUB NOTSHIP ALL]],
  collisionvolumeoffsets = [[0 -2 0]],
  collisionvolumescales = [[25 31 44]],
  collisionvolumetype = [[box]],
  defaultmissiontype = [[Standby]],
  description        = [[Chicken Assault]],
  explodeAs          = [[WEAVER_DEATH]],
  footprintX         = 3,
  footprintZ         = 3,
  iconType           = [[chicken]],
  leaveTracks        = false,
  maneuverleashlength = 640,
  mass               = 600,
  maxDamage          = 4500,
  maxVelocity        = 3.5999999046326,
  movementClass      = [[CHICKENHKBOT3]],
  name               = [[Enraged Lobber]],
  noAutoFire         = false,
  noChaseCategory    = [[VTOL]],
  objectName         = [[chickenr.s3o]],
  seismicSignature   = 4,
  selfDestructAs     = [[WEAVER_DEATH]],
  side               = [[THUNDERBIRDS]],
  sightDistance      = 390,
  smoothAnim         = false,
  steeringmode       = 2,
  TEDClass           = [[KBOT]],
  trackOffset        = 0,
  trackStrength      = 8,
  trackStretch       = 1,
  trackType          = [[ChickenTrack]],
  trackWidth         = 50,
  turninplace        = 0,
  turnRate           = 900,
  unitname           = [[chickenr2]],
  upright            = false,
  workerTime         = 0,
  featuredefs = {
    dead = {
      filename           = [[units/chickenr2.lua]],
    },
    heap = {
      filename           = [[units/chickenr2.lua]],
    },
  },
  sfxtypes = {
    explosiongenerators = {
      [[custom:blood_spray]],
      [[custom:blood_explode]],
      [[custom:dirt]],
    },
  },
  weapondefs = {
    chaseweapon = {
      craterboost        = 0,
      cratermult         = 0,
      endsmoke           = 0,
      explosiongenerator = [[custom:NONE]],
      impulseboost       = 0,
      impulsefactor      = 0,
      lineofsight        = [[true]],
      name               = [[Claws]],
      noselfdamage       = [[true]],
      proximitypriority  = -4,
      range              = 100,
      reloadtime         = 30,
      size               = 0.0010000000474975,
      startsmoke         = 0,
      targetborder       = 1,
      toairweapon        = 0,
      tolerance          = 5000,
      turret             = [[true]],
      weapontype         = [[Cannon]],
      weaponvelocity     = 500,
      damage = {
        default            = 0.0010000000474975,
      },
    },
    flamer = {
      areaofeffect       = 64,
      avoidfeature       = 0,
      avoidfriendly      = 0,
      burst              = 12,
      burstrate          = 0.0099999997764826,
      colormap           = [[0.2 0.5 0.9 0.7 0.2 0.5 0.9 0]],
      craterboost        = 0,
      cratermult         = 0,
      endsmoke           = 0,
      firestarter        = 100,
      flamegfxtime       = 1.8999999761581,
      groundbounce       = 1,
      impulseboost       = 0,
      impulsefactor      = 0,
      intensity          = 0.89999997615814,
      lineofsight        = [[true]],
      name               = [[FlameThrower]],
      noselfdamage       = [[true]],
      proximitypriority  = 4,
      randomdecay        = 0.60000002384186,
      range              = 310,
      reloadtime         = 0.69999998807907,
      rendertype         = 5,
      sizegrowth         = 1.2000000476837,
      smokedelay         = 1,
      soundstart         = [[Flamhvy1]],
      soundtrigger       = 0,
      sprayangle         = 9600,
      startsmoke         = 0,
      targetmoveerror    = 0.0010000000474975,
      toairweapon        = 0,
      tolerance          = 2500,
      turret             = [[true]],
      weapontimer        = 1,
      weapontype         = [[Flame]],
      weaponvelocity     = 300,
      damage = {
        chicken            = 6,
        default            = 40,
        tinychicken        = 3,
      },
    },
  },
  weapons = {
    [1]  = {
      badTargetCategory  = [[VTOL]],
      def                = [[chickenr2_chaseweapon]],
      mainDir            = [[0 0 1]],
      onlyTargetCategory = [[NOTAIR]],
    },
    [2]  = {
      badTargetCategory  = [[VTOL]],
      def                = [[chickenr2_flamer]],
      mainDir            = [[0 0 1]],
      maxAngleDif        = 120,
      onlyTargetCategory = [[NOTAIR]],
    },
  },
}


--------------------------------------------------------------------------------

local weaponDefs = {
  chickenr2_chaseweapon = {
    craterBoost        = 0,
    craterMult         = 0,
    endsmoke           = 0,
    explosionGenerator = [[custom:NONE]],
    impulseBoost       = 0,
    impulseFactor      = 0,
    lineOfSight        = false,
    name               = [[Claws]],
    noSelfDamage       = false,
    proximityPriority  = -4,
    range              = 100,
    reloadtime         = 30,
    size               = 0.0010000000474975,
    startsmoke         = 0,
    targetBorder       = 1,
    toAirWeapon        = false,
    tolerance          = 5000,
    turret             = true,
    weaponType         = [[Cannon]],
    weaponVelocity     = 500,
    damage = {
      default            = 0.0010000000474975,
    },
  },
  chickenr2_flamer = {
    areaOfEffect       = 64,
    avoidFeature       = false,
    avoidFriendly      = false,
    burst              = 12,
    burstrate          = 0.0099999997764826,
    colormap           = [[0.2 0.5 0.9 0.7 0.2 0.5 0.9 0]],
    craterBoost        = 0,
    craterMult         = 0,
    endsmoke           = 0,
    fireStarter        = 100,
    flameGfxTime       = 1.8999999761581,
    groundbounce       = true,
    impulseBoost       = 0,
    impulseFactor      = 0,
    intensity          = 0.89999997615814,
    lineOfSight        = false,
    name               = [[FlameThrower]],
    noSelfDamage       = false,
    proximityPriority  = 4,
    randomdecay        = 0.60000002384186,
    range              = 310,
    reloadtime         = 0.69999998807907,
    renderType         = 5,
    sizeGrowth         = 1.2000000476837,
    smokedelay         = 1,
    soundStart         = [[Flamhvy1]],
    soundTrigger       = false,
    sprayAngle         = 9600,
    startsmoke         = 0,
    targetMoveError    = 0.0010000000474975,
    toAirWeapon        = false,
    tolerance          = 2500,
    turret             = false,
    weaponTimer        = 1,
    weaponType         = [[Flame]],
    weaponVelocity     = 300,
    damage = {
      chicken            = 6,
      default            = 40,
      tinychicken        = 3,
    },
  },
}
unitDef.weaponDefs = weaponDefs


--------------------------------------------------------------------------------

return lowerkeys({ [unitName] = unitDef })

--------------------------------------------------------------------------------
