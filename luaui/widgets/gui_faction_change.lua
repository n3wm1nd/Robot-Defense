    ...
    if mx >= px and my >= py and my < py + 80 then
        ...
        if mButton == 1 then
            ...
            elseif mx < px + 128 then
                if spGetTeamRulesParam(myTeamID, 'startUnit') ~= 433 then --433
                    spSendLuaRulesMsg('\138433')
		    		      -- don't use caching, so we're sure the function has been loaded, also, it's called so rarely that doesn't matter if it's slow 
		      if WG["faction_change"] then 
			 WG["faction_change"](433) 
	           end 
                end
                return true
            elseif mx < px + 192 then
                if spGetTeamRulesParam(myTeamID, 'startUnit') ~= 761 then
                    spSendLuaRulesMsg('\138761')
		    		      -- don't use caching, so we're sure the function has been loaded, also, it's called so rarely that doesn't matter if it's slow 
		      if WG["faction_change"] then 
			 WG["faction_change"](761) 
	           end 
                end
                return true
            end
